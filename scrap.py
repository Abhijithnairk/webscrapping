...
for i in range(0,len(abmart_df)):
    search_item=abmart_df.loc[i]['name']
    URL = f"https://shop.sprouts.com/api/v2/store_products?ads_enabled=true&ads_pagination_improvements=true&allow_autocorrect=true&limit=1&offset=0&search_is_autocomplete=true&search_provider=ic&search_term={search_item}&secondary_results=true&sort=rank&unified_search_shadow_test_enabled=false"
    ...
    cleaned_items_df.to_excel(f'{search_item}.xlsx',index=False)